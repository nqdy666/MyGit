...
var serve_page = function(req, res) {
    var core_url = req.parsed_url.pathname;
    var page = core_url.substring(7);
    console.log(page);

    if (page !== "home") {
        page = core_url.substring(7, 12);
        if (page !== "album") {
            send_failure(res, 404, invalid_resource());
            return;
        }
...
var handle_incoming_request = function(req, res) {
    ...
    if (core_url.substring(0, 7) === "/pages/") {
        ...
        server_static_file("templates/" + core_url.substring(11), res);
    } else if (core_url.substring(0, 9) === "/content/") {
        server_static_file("content/" + core_url.substring(9), res);
    } else if (core_url === "/albums.json" && req.method.toLowerCase() === "get") {
        ...
        server_static_file("albums/" + core_url.substring(7), res);
    } else {
        send_failure(res, 404, invalid_resource());
    }
};
